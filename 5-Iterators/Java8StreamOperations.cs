namespace Iterators
{
    ...
    public static class Java8StreamOperations
    {
        ...
        {
            foreach(TAny item in sequence) consumer(item);
        }
        ...
        {
            foreach(TAny item in sequence) yield return mapper(item);
        }
        ...
        {
            IEnumerator<TAny> tmp = sequence.GetEnumerator();
            while(tmp.MoveNext())
            {
            ...
            while(tmp.MoveNext()) yield return tmp.Current;
        }
        ...
        public static IEnumerable<TAny> SkipSome<TAny>(this IEnumerable<TAny> sequence, long count)
        {
            IEnumerator<TAny> tmp = sequence.GetEnumerator();
            long i = 0;
            while(tmp.MoveNext() && i < count - 1) i++;
            while(tmp.MoveNext()) yield return tmp.Current;
        }
        ...
        {
            for(IEnumerator<TAny> tmp = sequence.GetEnumerator(); tmp.MoveNext() && predicate(tmp.Current);)
            {
                yield return tmp.Current;
            }
        }
